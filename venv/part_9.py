...
def is_palindrome(word)->bool:
    if type(word)!=str: # проверка на тип входных данных (для обработки чисел)
        word = str(word)
...
def dean_office(students_info, need_to):
    ...
    if need_to == 'best_marks_in_group':
        ...
        for student in students_info:
            ...
            if group in best_marks_in_group:
                if best_marks_in_group[group][1] < sum(marks) / len(marks): # best_marks_in_group[group][1] - 1 отвечает за местоположение средней оценки в группе
    ...
    if need_to == 'mean_marks':
        ...
        for group in marks_list:
            i=1
            for marks in zip(*marks_list[group]): # не особо эффективно из-за вложенного цикла, но другого решения не нашёл
                group_marks[group].append(['Предмет '+str(i) ,sum(marks)/len(marks)])
                i+=1
